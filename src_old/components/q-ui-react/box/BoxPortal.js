...
import EmailForm from '../../downshift/gmail';

const CtxPortal = React.createContext();
const ProvPortal = CtxPortal.Provider;

const BoxPortal = (Child) => {
	return class extends Component{ // export default 
		constructor(props){
			super(props);
			this.state = {
				boxWidth: null,// 250
		...
		//componentDidMount(){
			//console.log('componentDidMount in BoxPortal - src\\components\\q-ui-react\\box\\BoxPortal');
			//this.setState({boxWidth: 512}); // this.props.boxWidth
		//}
		...
		componentDidUpdate(prevProps, prevState, snapshot){
			//console.log('componentDidUpdate in BoxPortal - src\\components\\q-ui-react\\box\\BoxPortal');
		...
		dispatch = (action, e, data) => {
			switch (action.type){
				case "SET_BOX_WIDTH":
					return this.setState({boxWidth: data});
					//break;
				//case "SET_TITLE":
					//return this.setState({titleBegin: data});
					//break;
				case "OPEN_BOX":
					return this.onOpenBox(e, data);
					//break; 
				default: 
					break;
			}
		...
		onOpenBox = (e, data) => {
			...
			if(boxID.indexOf(boxLengthIndex) !== -1){ // boxID.indexOf(toHref) !== -1
				...
				const newBox = {
					boxExp: false,
					boxMin: false,
					boxTitle: data.title, // "New", // this.props.titleBegin, // "New Message", // targetTxt,
					id: 'box-'+ boxLengthIndex // toHref // Date.now()
				};
	...
	/*onScrollBox(e){
		...
		footBox = (type) => {
			if(type === 'chat'){
				...
				return (
					<div className="d-flex justify-content-between flexno border-top">
						<Btn size="sm" kind="primary" tip="Send email">SEND</Btn>
						<Btn outline size="sm" className="i ion-ios-trash scale12" type="reset" tip="Remove" />
					</div>
				);
			}
		}
		...
		render(){
			const {boxItems, boxItemsMax, boxMax, boxExpState, editorState, } = this.state;// boxLengthIndex
				...
				>
					...
					>
						...
						<Box
							...
							foot={this.footBox()}// FOR box footer
						...
						>
							{boxItems.length > 0 && 
								<Fragment>
									...
									<BraftEditor
										className="bg-white" //  border-bottom border
										controlBarClassName="bg-light border-top" 
...
export {BoxPortal, ConsumerPortal};

/*BoxPortal.defaultProps = {
  boxWidth: 512 // 250
};
...
//const Consumer = CtxPortal.Consumer;
