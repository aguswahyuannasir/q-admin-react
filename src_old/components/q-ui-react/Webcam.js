...
// import {Button} from 'reactstrap';// ButtonGroup,
...
export default class Webcam extends Component{
  ...
  componentDidMount(){
    ...
    if(!this.state.hasUserMedia && !this.userMediaRequested){
      this.requestUserMedia();
    }
  }

  shouldComponentUpdate(nextProps){ // componentWillUpdate
    if(JSON.stringify(nextProps.audioConstraints) !== JSON.stringify(this.props.audioConstraints) ||
  ...
  componentWillUnmount(){
    const index = this.mountedInstances.indexOf(this);
    this.mountedInstances.splice(index,1);

    this.userMediaRequested = false;
    if(this.mountedInstances.length === 0 && this.state.hasUserMedia){
  ...
  getScreenshot(){
    const me = this;
    if(!me.state.hasUserMedia) return null;
    ...
    if(canvas){
      return new Promise(function(resolve,reject){
        canvas.toBlob((blob) => {
          resolve(blob)
        },me.props.screenshotFormat,me.props.screenshotQuality)
      })
    }
  ...
  getCanvas(){
    ...
    if(!this.ctx){
      ...
      let canvasWidth = this.props.minScreenshotWidth || this.video.clientWidth;
    ...
    ctx.drawImage(this.video,0,0,canvas.width,canvas.height);// ,0,0,canvas.width,canvas.height
  ...
  render(){
    const {elClass, videoClass, w, h, src, audio, style, isShot} = this.props;

    return (
      <div className={classNames("webcam",elClass)}>
        <video
          onContextMenu={this.noop}
          className={classNames(videoClass,{"d-none": isShot})}// embed-responsive-item
          autoPlay
          width={w}
          height={h}
          src={src}
...
Webcam.defaultProps = {
  ...
  onUserMedia: () => {},
  onUserMediaError: () => {},
  screenshotFormat: 'image/png',// DEFAULT = 'image/webp'
  screenshotQuality: 0.95 // DEFAULT 0.92
};
...
Webcam.propTypes = {
  ...
  elClass: PropTypes.string,// className
