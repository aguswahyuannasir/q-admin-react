...
import Switch from '../../../components/common/Switch';// 'components/common/Switch'
import {imageControlItems} from '../../../configs/controls';// 'configs/controls'
// import CheckBox from '../../../../q-ui-react/CheckBox';

export default class Image extends React.Component{
  state = {
    toolbarVisible: false,// true
    toolbarOffset: 0,
    ...
    const renderedControlItems = imageControls.map((item, index) => {
      if(typeof item === 'string' && imageControlItems[item]){
        return (
          <div 
						key={index}
						className={item === 'link' && link ? 'active' : undefined} 
						onClick={() => this.executeCommand(imageControlItems[item].command)}>
            {imageControlItems[item].text}
          </div>
        )
      }else if(item && (item.render || item.text)){
        return item.render ? item.render(mediaData, this.props.block) : <div key={index} onClick={() => item.onClick && this.executeCommand(item.onClick)}>{item.text}</div>
      }else{
        return null
      }
    })
            ...
            >
                ...
                <div className="bf-image-link-editor">
                  <div className="editor-input-group">
                    <input type="text" placeholder={language.linkEditor.inputWithEnterPlaceHolder} 
											title={language.linkEditor.inputWithEnterPlaceHolder} 
											onKeyDown={this.handleLinkInputKeyDown} 
											onChange={this.setImageLink} defaultValue={link}/>
                  ...
                  <div className="switch-group">
                    ...
                    >
											<label className="mb-0 cpoin">{language.linkEditor.openInNewWindow}</label>
										</Switch>
                  </div>
                </div>
              ) : null}
              ...
              {sizeEditorVisible ? (
                <div className="bf-image-size-editor">
                  <div className="editor-input-group">
                    <input type="text" onDragStart={this.noop} placeholder={language.base.width} onKeyDown={this.handleSizeInputKeyDown} onChange={this.setImageWidth} defaultValue={width}/>
                    <input type="text" onDragStart={this.noop} placeholder={language.base.height} onKeyDown={this.handleSizeInputKeyDown} onChange={this.setImageHeight} defaultValue={height}/>
