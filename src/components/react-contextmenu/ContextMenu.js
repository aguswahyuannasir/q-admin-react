...
export default class ContextMenu extends AbstractMenu{
	...
	registerHandlers = () => {
		...
		document.addEventListener("focusout",this.hideMenu);// Q-CUSTOM
	...
	unregisterHandlers = () => {
		...
		document.removeEventListener("focusout",this.hideMenu);// Q-CUSTOM
...
// Q-CUSTOM
	...
	handleContextMenu = e => {
		...
		/*if(process.env.NODE_ENV === 'production'){ // DEFAULT
			e.preventDefault();
		}*/
		e.preventDefault();// Q-CUSTOM
		this.handleHide(e);
	}

	hideMenu = e => {
		if((e.keyCode === 27 || e.keyCode === 13) || !document.hasFocus()){ // ECS or enter
			hideMenu();
		}
	}
