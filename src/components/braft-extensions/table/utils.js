...
// Metode perbandingan nilai sederhana
...
// 创建并返回一行单元格block - Buat dan kembalikan baris blok sel
...
// 将表格block更新到contentState - Perbarui blok tabel ke contentState
...
// 使用简易值比较函数筛选符合条件的block - Gunakan fungsi perbandingan nilai sederhana untuk memfilter blok yang memenuhi syarat
...
// 遍历以修正单元格的colIndex属性（表格blocks专用） - Traverse untuk memperbaiki properti colIndex sel (untuk blok tabel)
...
// 遍历以修正单元格的colSpan和rowSpan属性（表格DOM专用） - Traverse untuk memperbaiki properti colSpan dan rowSpan sel (untuk tabel DOM)
...
// 获取需要插入到某一行的单元格的数量 - Dapatkan jumlah sel yang perlu dimasukkan ke dalam satu baris
...
// 获取指定范围内的单元格block - Dapatkan blok sel dalam rentang yang ditentukan
...
// 插入一个单元格block到表格的block列表中 - Masukkan blok sel ke dalam daftar blokir tabel
...
// 插入多个单元格block到表格的block列表中 - Masukkan beberapa blok sel ke dalam daftar blokir tabel
...
// 插入一列单元格到表格中 - Masukkan kolom sel ke dalam tabel
...
// 从表格中移除指定的某一列单元格 - Hapus kolom sel tertentu dari tabel
...
// 插入一行单元格到表格中 - Masukkan baris sel ke dalam tabel
...
// 从表格中移除指定的某一行单元格 - Hapus baris sel tertentu dari tabel
