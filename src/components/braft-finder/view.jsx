...
import {UniqueIndex} from './utils/base';// 'utils/base'
...
export default class BraftFinderView extends React.Component {
        ...
        <footer className="bf-manager-footer">
          <div className="pull-left">
            {allowExternal ? (
              ...
              >
                {showExternalForm ? (
                  <span className="bf-bottom-text"><i className="braft-icon-add" /> {language.addLocalFile}</span>
                ) : (
                  <span className="bf-bottom-text"><i className="braft-icon-add" /> {language.addExternalSource}</span>
                )}
              </span>
            ) : null}
          </div>
        ...
        {this.state.items.map((item, index) => {
          ...
          let progressMarker = item.uploading && !this.props.hideProgress ? (
            <div className="bf-item-uploading">
              <div className="bf-item-uploading-bar" style={{width: item.uploadProgress / 1 + '%'}} />
            </div>
          ) : ''
            ...
            case 'IMAGE':
              previewerComponents = (
                <div className="bf-image">
                  {progressMarker}
                  <img src={item.thumbnail || item.url} alt="img_preview" draggable="false" />
                </div>
              )
            break
            case 'VIDEO':
          ...
          return (
            ...
            >
              {previewerComponents}
              <span data-id={item.id} onClick={this.removeItem} className="bf-item-remove braft-icon-close" />
  ...
  reslovePickedFiles = (e) => {
    ...
    this.controller.resolveFiles({
      ...
      onAllReady: () => e.target.value = null
    }, 0, accepts)
  }
