...
// import {UncontrolledPopover, PopoverHeader, PopoverBody} from 'reactstrap';// Col
					...
					{(menus && menus.length > 0) && 
						menus.map((v, i) => 
							v.kids && v.kids.length > 0 ? 
								...
								:
								<div key={i} id={`menu-${v.id}`}>
									<A dom href={v.url} aClass="nav-link">{v.txt}</A>
...
/* 
									<UncontrolledPopover trigger="hover" placement="right-start" target={`menu-${v.id}`}>
										<PopoverHeader>Popover Title</PopoverHeader>
										<PopoverBody>Venenatis vestibulum.</PopoverBody>
									</UncontrolledPopover>
